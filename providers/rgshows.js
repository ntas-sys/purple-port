...
var __origGetStreams_rgshows = getStreams;
getStreams = function (tmdbId, mediaType, season, episode) {
  return __origGetStreams_rgshows(tmdbId, mediaType, season, episode)
    .then(function (streams) {
      if (!streams || streams.length === 0) return [];
      console.log('[extractor] rgshows: ' + streams.length + ' embed bulundu, extracting...');
      ...
      var providerName = streams[0].name || 'rgshows';
      return extractAllStreams(embedUrls, referer, providerName);
    })
    .catch(function (err) {
      console.error('[extractor] rgshows hata: ' + (err && err.message ? err.message : err));
      return [];
    });
};
