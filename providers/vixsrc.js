...
var __origGetStreams_vixsrc = getStreams;
getStreams = function (tmdbId, mediaType, season, episode) {
  return __origGetStreams_vixsrc(tmdbId, mediaType, season, episode)
    .then(function (streams) {
      if (!streams || streams.length === 0) return [];
      console.log('[extractor] vixsrc: ' + streams.length + ' embed bulundu, extracting...');
      ...
      var providerName = streams[0].name || 'vixsrc';
      return extractAllStreams(embedUrls, referer, providerName);
    })
    .catch(function (err) {
      console.error('[extractor] vixsrc hata: ' + (err && err.message ? err.message : err));
      return [];
    });
};
