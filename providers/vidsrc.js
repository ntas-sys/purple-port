...
var __origGetStreams_vidsrc = getStreams;
getStreams = function (tmdbId, mediaType, season, episode) {
  return __origGetStreams_vidsrc(tmdbId, mediaType, season, episode)
    .then(function (streams) {
      if (!streams || streams.length === 0) return [];
      console.log('[extractor] vidsrc: ' + streams.length + ' embed bulundu, extracting...');
      ...
      var providerName = streams[0].name || 'vidsrc';
      return extractAllStreams(embedUrls, referer, providerName);
    })
    .catch(function (err) {
      console.error('[extractor] vidsrc hata: ' + (err && err.message ? err.message : err));
      return [];
    });
};
